...
FluidSystem::FluidSystem ()
{
	...
	/*if ( !xml.Load ( "scene.xml" ) ) {
		dbgprintf ( "fluid", "ERROR: Problem loading scene.xml. Check formatting.\n" );
		exit(-1);
	}*/
...
void FluidSystem::record ( int param, std::string name, TimeX& start )
{
	TimeX stop;
	stop.SetSystemTime ();
	stop = stop - start;
...
void FluidSystem::RunValidate ()
{
	...
	dbgprintf ( "CPU:\n"); for (n=0, c=0; n < NumPoints() && c < 20; n++) {dbgprintf ( "p: %d, cell: %d, cluster: %d\n", n, mGridCell[n], mClusterCell[n] ); c++;} 
	...
	dbgprintf ( "CPU:\n"); for (n=0, c=0; n < m_GridTotal && c < 20; n++) if ( cpu_gridcnt[n]!=0 ) {dbgprintf ( "cell: %d, cnt: %d\n", n, (int) cpu_gridcnt[n] );c++;}
	dbgprintf ( "GPU:\n"); for (n=0, c=0; n < m_GridTotal && c < 20; n++) if ( gpu_gridcnt[n]!=0 ) {dbgprintf ( "cell: %d, cnt: %d\n", n, gpu_gridcnt[n] );c++;}	
	...
	dbgprintf ( "CPU:\n"); for (n=0, c=0; n < m_GridTotal && c < 20; n++) if ( cpu_gridcnt[n]!=0 ) {dbgprintf ( "cell: %d, cnt: %d, off: %d\n", n, (int) cpu_gridcnt[n], cpu_gridoff[n] );c++;}
	dbgprintf ( "GPU:\n"); for (n=0, c=0; n < m_GridTotal && c < 20; n++) if ( gpu_gridcnt[n]!=0 ) {dbgprintf ( "cell: %d, cnt: %d, off: %d\n", n, gpu_gridcnt[n], gpu_gridoff[n] );c++;}	
...
void FluidSystem::Advance ()
{
	...
	for ( int n=0; n < NumPoints(); n++ ) {
		...
		if ( m_Param[PPOINT_GRAV_AMT] > 0 ) {
			...
			norm.Normalize ();
			norm *= (float) m_Param[PPOINT_GRAV_AMT];
			accel -= norm;
		}
...
//	Setup ();
	...
	Run (0,0);			dbgprintf ( "  New Algorithm. OK\n" );
...
void FluidSystem::Draw ( Camera3D* cam, float rad )
{
	...
	case 0: {
		...
		glBufferData ( GL_ARRAY_BUFFER, NumPoints()*sizeof(Vector3DF), mPos, GL_DYNAMIC_DRAW );		
		glVertexAttribPointer ( 0, 3, GL_FLOAT, GL_FALSE, sizeof(Vector3DF), 0 );			
		checkGL ( "bind pos" ); 
		
		glBindBuffer ( GL_ARRAY_BUFFER, mVBO[1] );		
		glBufferData ( GL_ARRAY_BUFFER, NumPoints()*sizeof(uint),  mClr, GL_DYNAMIC_DRAW_ARB);
...
void FluidSystem::StartRecord ()
{
	...
	mFP = fopen ( name, "wb" );		
	if ( mFP == 0x0 ) {
		dbgprintf ( "ERROR: Cannot write file %s\n", mFileName.c_str() );
		exit ( -1 );
	}
	mLastPoints = 0;
	mFileSize = 0;
...
void FluidSystem::StartPlayback ( int p )
{
	...
	if ( mFP==0x0 || ferror(mFP) ) {
		dbgprintf ( "ERROR: Cannot read file %s\n", mFileName.c_str() );
		perror ( "  " );
		exit ( -1 );
	}
	m_Frame = 0;
...
void FluidSystem::DrawParticleInfo ( int p )
{
	...
	if ( cc != -1 ) {
		...
		for (int n=0; n < m_GridAdjCnt; n++ ) {		// Cluster group
			cadj = cc+m_GridAdj[n];
			gc = getCell ( cadj );
			sprintf ( disp, "<%d, %d, %d> id: %d, cnt: %d ", gc.x, gc.y, gc.z, cc+m_GridAdj[n], m_GridCnt[ cadj ] );	drawText ( 20, 100+n*10, disp, 1,1,1,1 );
...
void FluidSystem::SetupDefaultParams ()
{
	...
	m_Param [ PSIMSCALE ] =		0.007;			// unit size
	...
	m_Param [ PRADIUS ] =		0.015;			// m
	m_Param [ PDIST ] =			0.0059;			// m
	m_Param [ PSMOOTHRADIUS ] =	0.015;			// m 
	...
	m_Param [PDRAWMODE] = 0;				// Sprite drawing
...
void FluidSystem::SetupSpacing ()
{
	...
	// Particle Boundaries
	m_Vec[PBOUNDMIN] = m_Vec[PVOLMIN];		m_Vec[PBOUNDMIN] += float(2.0f*(m_Param[PGRIDSIZE] / m_Param[PSIMSCALE]));
	m_Vec[PBOUNDMAX] = m_Vec[PVOLMAX];		m_Vec[PBOUNDMAX] -= float(2.0f*(m_Param[PGRIDSIZE] / m_Param[PSIMSCALE]));
}
...
void FluidSystem::TestPrefixSum ( int num )
{
	...
	dbgprintf ( "CPU:   "); for (int n=num-10; n < num; n++)	printf ( "%d ", listOutCPU[n] ); printf (" (last 10 values)\n");		// print first 10
