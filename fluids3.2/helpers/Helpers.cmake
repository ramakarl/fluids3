...
####################################################################################
# CMAKE HELPERS - GLOBAL START

# Helpers path
# *NOTE*: The helpers path is set by CMakeLists.txt as ${HELPERS_DIR}
#
set( EXECUTABLE_OUTPUT_PATH ${CMAKE_CURRENT_BINARY_DIR} CACHE PATH "Executable path" )

# Helpers source
if ( NOT DEFINED HELPERS_PATH ) 
  get_filename_component ( _helpersrc "${HELPERS_DIR}" REALPATH )
  set ( HELPERS_PATH ${_helpersrc} CACHE PATH "Full path to /helpers" )
endif()

# Combined libraries
get_filename_component ( _combined  "${HELPERS_PATH}" REALPATH )

set ( COMBINED_ROOT_DIR ${_combined} CACHE PATH "Path to combined libraries.")

set ( COMBINED_INCLUDE_PATH "${_combined}" CACHE PATH "Combined includes.")

set ( COMBINED_LIB_PATH "${_combined}" CACHE PATH "Combined libs.")

include_directories( "${COMBINED_INCLUDE_PATH}" )	
...
message ( "HELPERS_DIR:        ${HELPERS_DIR}" )
message ( "COMBINED_ROOT_DIR:  ${COMBINED_ROOT_DIR}" )
message ( "COMBINED_LIB_PATH:  ${COMBINED_LIB_PATH}" )
message ( "COMBINED_INC_PATH:  ${COMBINED_INCLUDE_PATH}" )
...
FUNCTION( _LINKPTX )
  set(options "")
  set(oneValueArgs SRC OUTPUT )  
  set(multiValueArgs INPUTS OPTIONS SOURCES)
  CMAKE_PARSE_ARGUMENTS( _LINKPTX "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})
  string (REPLACE ";" " " _LINKPTX_OPTIONS "${_LINKPTX_OPTIONS}")  
  string (REPLACE ";" " " _LINKPTX_INPUTS "${_LINKPTX_INPUTS}")  
  separate_arguments( _OPTS WINDOWS_COMMAND "${_LINKPTX_OPTIONS}" )      
  separate_arguments( _INPUTS WINDOWS_COMMAND "${_LINKPTX_INPUTS}" )      
  set ( INCL "-I\"${_LINKPTX_INCLUDE}\"" )  

  if ( WIN32 ) 
		message( STATUS "NVCC Link: ${CUDA_NVCC_EXECUTABLE} ${_LINKPTX_OPTIONS} ${_INPUTS} -o ${_LINKPTX_OUTPUT} WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}")
		add_custom_command(
			OUTPUT ${_LINKPTX_OUTPUT}
			MAIN_DEPENDENCY ${_LINKPTX_SRC}			
			DEPENDS ${_INPUTS}
			COMMAND ${CUDA_NVCC_EXECUTABLE} ${_OPTS} ${_INPUTS} -o ${_LINKPTX_OUTPUT} WORKING_DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}"
		)
  endif()
...
FUNCTION( _COMPILEPTX )
  set(options "")
  set(oneValueArgs TARGET_PATH GENERATED GENPATHS INCLUDE OUTEXT)  
  ...
  if ( WIN32 ) 
		...
		FOREACH( input ${_COMPILEPTX_SOURCES} )
			...
			if ( ${input_ext} STREQUAL ".cu" OR ${input_ext} STREQUAL ".obj" )			
				
				# Set output names
				set( output "${input_without_ext}.${OUTEXT}" )							# Output name
				set( output_with_path "${_COMPILEPTX_TARGET_PATH}/$(Configuration)/${input_without_ext}.${OUTEXT}" )	# Output with path
				...
				message( STATUS "NVCC Compile: ${CUDA_NVCC_EXECUTABLE} ${MACHINE} ${_COMPILEPTX_OPTIONS} ${input} ${INCL} -o ${output_with_path} WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}")
    
				add_custom_command(
					OUTPUT  ${output_with_path}
					MAIN_DEPENDENCY ${input}
					COMMAND ${CUDA_NVCC_EXECUTABLE} ${MACHINE} ${_OPTS} ${input} ${INCL} -o ${output_with_quote} WORKING_DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}"
				)
			endif()
		ENDFOREACH( )
  else ()
